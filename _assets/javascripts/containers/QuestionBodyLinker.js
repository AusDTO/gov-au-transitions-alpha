...
const getQuestion = (index, questions, answers, language) => {
  ...
  return {
    ...
    legend: getQuestionLegend(questions[index].legend, language),
...
const mapStateToProps = ({ router, currentQuestion, currentAnswers, language }) => {
  let index = router.params && router.params.id ? getIndexFromId(router.params.id) : 0
  return getQuestion(index, QuestionFlow.questions, currentAnswers, language)
}
